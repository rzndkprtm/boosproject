...
Partial Class Order_Upload
    ...
    Protected Sub btnSubmit_Click(sender As Object, e As EventArgs)
        ...
        Try
            ...
            If Not (fileExtension = ".xls" OrElse fileExtension = ".xlsx") Then
                MessageError(True, "THE FILE FORMAT IS NOT SUPPORTED. PLEASE UPLOAD A VALID FILE. !")
                Exit Sub
            End If

            If msgError.InnerText = "" Then
                Dim fileName As String = Path.GetFileName(fuFile.FileName)
                Dim fileBytes As Byte() = fuFile.FileBytes

                Using uploadStream As New MemoryStream(fileBytes)
                    Using package As New ExcelPackage(uploadStream)
                        ...
                        If ddlCustomer.SelectedValue = "127" Then
                            If orderContainer = "NZN" OrElse orderContainer = "NZS" Then
                                aliasCustomerId = "1614"
                            End If
                        End If
                        ...
                        Using orderItemStream As New MemoryStream(fileBytes)
                            Using orderItem As New ExcelPackage(orderItemStream)
                                ...
                                For row As Integer = startRow To lastRow
                                    Dim designType As String = If(sheetDetail.Cells(row, 1).Text IsNot Nothing,
                                       sheetDetail.Cells(row, 1).Text,
                                       "")
                                    ...
                                    If designType = "Vertical" Then
                                        ...
                                        Dim tubeId As String = orderClass.GetItemData("SELECT Id FROM ProductTubes WHERE Alias='" & tubeName & " (Vertical)" & "'")
                                        ...
                                        Dim productId As String = orderClass.GetItemData("SELECT Id FROM Products CROSS APPLY STRING_SPLIT(CompanyDetailId, ',') AS companyArray WHERE DesignId='" & designId & "' AND BlindId='" & blindId & "' AND companyArray.VALUE='" & companyDetailId & "' AND TubeType='" & tubeId & "' AND ControlType='" & controlId & "' AND ColourType='" & colourId & "' AND (Status='In Stock' OR Status='Limited Stock')")
                                        ...
                                        If msgError.InnerText = "" Then
                                            ...
                                            Using thisConn As SqlConnection = New SqlConnection(myConn)
                                                Using thisCmd As SqlCommand = New SqlCommand("INSERT INTO OrderDetails(Id, HeaderId, ProductId, FabricId, FabricColourId, ChainId, PriceProductGroupId, Qty, QtyBlade, Room, Mounting, Width, [Drop], StackPosition, ControlPosition, ControlLength, ControlLengthValue, WandColour, WandLengthValue, FabricInsert, BottomJoining, BracketExtension, Sloping, LinearMetre, SquareMetre, TotalItems, Notes, MarkUp, Active) VALUES(@Id, @HeaderId, @ProductId, @FabricId, @FabricColourId, @ChainId, @PriceProductGroupId, @Qty, @QtyBlade, @Room, @Mounting, @Width, @Drop, @StackPosition, @ControlPosition, @ControlLength, @ControlLengthValue, @WandColour, @WandLengthValue, @FabricInsert, @BottomJoining, @BracketExtension, @Sloping, @LinearMetre, @SquareMetre, 1, @Notes, @MarkUp, 1)", thisConn)
                                                    ...
                                                    thisCmd.Parameters.AddWithValue("@FabricId", If(String.IsNullOrEmpty(fabricId), CType(DBNull.Value, Object), fabricId))
                                                    thisCmd.Parameters.AddWithValue("@FabricColourId", If(String.IsNullOrEmpty(fabricColourId), CType(DBNull.Value, Object), fabricColourId))
                                                    thisCmd.Parameters.AddWithValue("@ChainId", If(String.IsNullOrEmpty(chainId), CType(DBNull.Value, Object), chainId))
                                                    ...
                                                    thisCmd.Parameters.AddWithValue("@Room", room)
                                                    thisCmd.Parameters.AddWithValue("@Mounting", sizeType & " " & mounting)
                                                    thisCmd.Parameters.AddWithValue("@Width", width)
                                                    thisCmd.Parameters.AddWithValue("@Drop", drop)
                                                    thisCmd.Parameters.AddWithValue("@FabricInsert", fabricInsert)
                                                    thisCmd.Parameters.AddWithValue("@StackPosition", stackPosition)
                                                    thisCmd.Parameters.AddWithValue("@ControlPosition", controlPosition)
                                                    ...
                                                    thisCmd.Parameters.AddWithValue("@WandColour", controlColour)
                                                    thisCmd.Parameters.AddWithValue("@WandLengthValue", wandLengthValue)
                                                    thisCmd.Parameters.AddWithValue("@BottomJoining", bottomJoining)
                                                    thisCmd.Parameters.AddWithValue("@BracketExtension", bracketExtension)
                                                    thisCmd.Parameters.AddWithValue("@Sloping", sloping)
                                                    thisCmd.Parameters.AddWithValue("@LinearMetre", linearMetre)
                                                    thisCmd.Parameters.AddWithValue("@SquareMetre", squareMetre)
                                                    thisCmd.Parameters.AddWithValue("@Notes", notes)
                                    ...
                                    If designType = "Cellular Shades" Then
                                        ...
                                        Dim productId As String = orderClass.GetItemData("SELECT Id FROM Products CROSS APPLY STRING_SPLIT(CompanyDetailId, ',') AS companyArray WHERE DesignId='" & designId & "' AND BlindId='" & blindId & "' AND companyArray.value='" & companyDetailId & "' AND TubeType='" & tubeId & "' AND ControlType='" & controlId & "' AND ColourType='56' AND (Status='In Stock' OR Status='Limited Stock')")
                                        If String.IsNullOrEmpty(productId) Then
                                            Dim thisAlert As String = String.Format("THE PRODUCT YOU REQUESTED IS CURRENTLY NOT AVAILABLE IN OUR STOCKS. PLEASE REVIEW YOUR ORDER OR CONTACT OUR CUSTOMER SERVICE TEAM FOR ASSISTANCE. ITEM {0} !", itemNumber)
                                            MessageError(True, thisAlert)
                                            Exit For
                                        ...
                                        If msgError.InnerText = "" Then
                                            ...
                                            Using thisConn As SqlConnection = New SqlConnection(myConn)
                                                Using thisCmd As SqlCommand = New SqlCommand("INSERT INTO OrderDetails(Id, HeaderId, ProductId, FabricId, FabricIdB, FabricColourId, FabricColourIdB, PriceProductGroupId, Qty, Room, Mounting, Width, WidthB, [Drop], DropB, ControlPosition, ControlLength, ControlLengthValue, Supply, LinearMetre, LinearMetreB, SquareMetre, SquareMetreB, TotalItems, Notes, MarkUp, Active) VALUES(@Id, @HeaderId, @ProductId, @FabricId, @FabricIdB, @FabricColourId, @FabricColourIdB, @PriceProductGroupId, @Qty, @Room, @Mounting, @Width, @WidthB, @Drop, @DropB, @ControlPosition, @ControlLength, @ControlLengthValue, @Supply, @LinearMetre, @LinearMetreB, @SquareMetre, @SquareMetreB, @TotalItems, @Notes, @MarkUp, 1)", thisConn)
                                                    ...
                                                    thisCmd.Parameters.AddWithValue("@Qty", "1")
                                                    thisCmd.Parameters.AddWithValue("@Room", room)
                                                    thisCmd.Parameters.AddWithValue("@FabricId", fabricId)
                                                    thisCmd.Parameters.AddWithValue("@FabricColourId", fabricColourId)
                                                    thisCmd.Parameters.AddWithValue("@FabricIdB", If(String.IsNullOrEmpty(fabricIdB), CType(DBNull.Value, Object), fabricIdB))
                                                    thisCmd.Parameters.AddWithValue("@FabricColourIdB", If(String.IsNullOrEmpty(fabricColourIdB), CType(DBNull.Value, Object), fabricColourIdB))
                                                    thisCmd.Parameters.AddWithValue("@Mounting", mounting)
                                                    thisCmd.Parameters.AddWithValue("@ControlPosition", controlPosition)
                                                    thisCmd.Parameters.AddWithValue("@Width", width)
                                                    ...
                                                    thisCmd.Parameters.AddWithValue("@Supply", supply)
                                                    thisCmd.Parameters.AddWithValue("@Notes", notes)
                                                    thisCmd.Parameters.AddWithValue("@MarkUp", 0)
                                    ...
                                    If designType = "Roman" Then
                                        ...
                                        Dim productId As String = orderClass.GetItemData("SELECT Id FROM Products CROSS APPLY STRING_SPLIT(CompanyDetailId, ',') AS companyArray WHERE DesignId='" & designId & "' AND BlindId='" & blindId & "' AND TubeType='" & tubeId & "' AND ControlType='" & controlId & "' AND ColourType='56' AND (Status='In Stock' OR Status='Limited Stock')")
                                        If String.IsNullOrEmpty(productId) Then
                                            Dim thisAlert As String = String.Format("THE PRODUCT YOU REQUESTED IS CURRENTLY NOT AVAILABLE IN OUR STOCKS. PLEASE REVIEW YOUR ORDER OR CONTACT OUR CUSTOMER SERVICE TEAM FOR ASSISTANCE. ITEM {0} !", itemNumber)
                                            MessageError(True, thisAlert)
                                            Exit For
                                        ...
                                        Using thisConn As SqlConnection = New SqlConnection(myConn)
                                            Using thisCmd As SqlCommand = New SqlCommand("INSERT INTO OrderDetails(Id, HeaderId, ProductId, FabricId, FabricColourId, ChainId, PriceProductGroupId, Qty, Room, Mounting, Width, [Drop], ControlPosition, ControlColour, ControlLength, ControlLengthValue, ValanceOption, Batten, LinearMetre, SquareMetre, TotalItems, Notes, MarkUp, Active) VALUES(@Id, @HeaderId, @ProductId, @FabricId, @FabricColourId, @ChainId, @PriceProductGroupId, @Qty, @Room, @Mounting, @Width, @Drop, @ControlPosition, @ControlColour, @ControlLength, @ControlLengthValue, @ValanceOption, @Batten, @LinearMetre, @SquareMetre, 1, @Notes, 0, 1)", thisConn)
                                                ...
                                                thisCmd.Parameters.AddWithValue("@Drop", drop)
                                                thisCmd.Parameters.AddWithValue("@ControlPosition", controlPosition)
                                                thisCmd.Parameters.AddWithValue("@ControlColour", controlColour)
                                                thisCmd.Parameters.AddWithValue("@ControlLength", controlLength)
                                                thisCmd.Parameters.AddWithValue("@ControlLengthValue", controlLengthValue)
                                                thisCmd.Parameters.AddWithValue("@ValanceOption", valanceOption)
                                                thisCmd.Parameters.AddWithValue("@Batten", batten)
                                                thisCmd.Parameters.AddWithValue("@LinearMetre", linearMetre)
                                                thisCmd.Parameters.AddWithValue("@SquareMetre", squareMetre)
                                                thisCmd.Parameters.AddWithValue("@Notes", notes)
                                    ...
                                    If designType = "Panel Glide" Then
                                        ...
                                        Dim productId As String = orderClass.GetItemData("SELECT Id FROM Products CROSS APPLY STRING_SPLIT(CompanyDetailId, ',') AS companyArray WHERE DesignId='" & designId & "' AND BlindId='" & blindId & "' AND TubeType='" & tubeId & "' AND ColourType='" & colourId & "' AND (Status='In Stock' OR Status='Limited Stock')")
                                        ...
                                        Using thisConn As New SqlConnection(myConn)
                                            Using thisCmd As New SqlCommand("INSERT INTO OrderDetails(Id, HeaderId, ProductId, FabricId, FabricColourId, PriceProductGroupId, Qty, Room, Mounting, Width, [Drop], WandColour, WandLength, WandLengthValue, LayoutCode, LayoutCodeCustom, TrackType, PanelQty, Batten, BattenB, LinearMetre, SquareMetre, TotalItems, Notes, Markup, Active) VALUES(@Id, @HeaderId, @ProductId, @FabricId, @FabricColourId, @PriceProductGroupId, @Qty, @Room, @Mounting, @Width, @Drop, @WandColour, @WandLength, @WandLengthValue, @LayoutCode, @LayoutCodeCustom, @TrackType, @PanelQty, @Batten, @BattenB, @LinearMetre, @SquareMetre, 1, @Notes, @MarkUp, 1)", thisConn)
                                                ...
                                                thisCmd.Parameters.AddWithValue("@Qty", 1)
                                                thisCmd.Parameters.AddWithValue("@Room", room)
                                                thisCmd.Parameters.AddWithValue("@Mounting", String.Format("{0} {1}", sizeType, mounting))
                                                thisCmd.Parameters.AddWithValue("@Width", width)
                                                thisCmd.Parameters.AddWithValue("@Drop", drop)
                                                thisCmd.Parameters.AddWithValue("@WandColour", wandColour)
                                                thisCmd.Parameters.AddWithValue("@WandLength", wandLength)
                                                thisCmd.Parameters.AddWithValue("@WandLengthValue", wandLengthValue)
                                                thisCmd.Parameters.AddWithValue("@LayoutCode", layoutCode)
                                                thisCmd.Parameters.AddWithValue("@LayoutCodeCustom", String.Empty)
                                                thisCmd.Parameters.AddWithValue("@TrackType", trackType)
                                                thisCmd.Parameters.AddWithValue("@PanelQty", panelQty)
                                                thisCmd.Parameters.AddWithValue("@Batten", batten)
                                                thisCmd.Parameters.AddWithValue("@BattenB", battenb)
                                                thisCmd.Parameters.AddWithValue("@LinearMetre", linearMetre)
                                                thisCmd.Parameters.AddWithValue("@SquareMetre", squareMetre)
                                                thisCmd.Parameters.AddWithValue("@Notes", notes)
                                                thisCmd.Parameters.AddWithValue("@MarkUp", 0)
                                        ...
                                        If msgError.InnerText = "" Then
                                            ...
                                            Using thisConn As SqlConnection = New SqlConnection(myConn)
                                                Using thisCmd As SqlCommand = New SqlCommand("INSERT INTO OrderDetails (Id, HeaderId, ProductId, PriceProductGroupId, SubType, Qty, Room, Mounting, ControlPosition, TilterPosition, Width, [Drop], Supply, Tassel, ControlLength, ControlLengthValue, WandLength, WandLengthValue, ValanceType, ValanceSize, ValanceSizeValue, ReturnPosition, ReturnLength, ReturnLengthValue, LinearMetre, SquareMetre, TotalItems, Notes, MarkUp, Active) VALUES (@Id, @HeaderId, @ProductId, @PriceProductGroupId, @SubType, 1, @Room, @Mounting, @ControlPosition, @TilterPosition, @Width, @Drop, @Supply, @Tassel, @ControlLength, @ControlLengthValue, @WandLength, @WandLengthValue, @ValanceType, @ValanceSize, @ValanceSizeValue, @ReturnPosition, @ReturnLength, @ReturnLengthValue, @LinearMetre, @SquareMetre, @TotalItems, @Notes, @MarkUp, 1)", thisConn)
                                                    ...
                                                    thisCmd.Parameters.AddWithValue("@PriceProductGroupId", If(String.IsNullOrEmpty(priceProductGroup), CType(DBNull.Value, Object), priceProductGroup))
                                                    thisCmd.Parameters.AddWithValue("@Room", room)
                                                    thisCmd.Parameters.AddWithValue("@Mounting", finalMounting)
                                                    thisCmd.Parameters.AddWithValue("@SubType", subType)
                                                    thisCmd.Parameters.AddWithValue("@ControlPosition", controlPosition)
                                                    thisCmd.Parameters.AddWithValue("@TilterPosition", tilterPosition)
                                                    thisCmd.Parameters.AddWithValue("@Width", width)
                                                    thisCmd.Parameters.AddWithValue("@Drop", drop)
                                                    ...
                                                    thisCmd.Parameters.AddWithValue("@ValanceType", valanceType)
                                                    thisCmd.Parameters.AddWithValue("@ValanceSize", valanceSize)
                                                    thisCmd.Parameters.AddWithValue("@ValanceSizeValue", valanceSizeValue)

                                                    thisCmd.Parameters.AddWithValue("@ReturnPosition", returnPosition)
                                                    thisCmd.Parameters.AddWithValue("@ReturnLength", returnLength)
                                                    ...
                                                    thisCmd.Parameters.AddWithValue("@Tassel", tassel)
                                                    thisCmd.Parameters.AddWithValue("@Supply", supply)
                                        ...
                                        If msgError.InnerText = "" Then
                                            ...
                                            Using thisConn As SqlConnection = New SqlConnection(myConn)
                                                Using thisCmd As SqlCommand = New SqlCommand("INSERT INTO OrderDetails(Id, HeaderId, ProductId, FabricId, FabricIdB, FabricIdC, FabricIdD, FabricIdE, FabricIdF, FabricColourId, FabricColourIdB, FabricColourIdC, FabricColourIdD, FabricColourIdE, FabricColourIdF, ChainId, ChainIdB, ChainIdC, ChainIdD, ChainIdE, ChainIdF, BottomId, BottomIdB, BottomIdC, BottomIdD, BottomIdE, BottomIdF, BottomColourId, BottomColourIdB, BottomColourIdC, BottomColourIdD, BottomColourIdE, BottomColourIdF, PriceProductGroupId, PriceProductGroupIdB, PriceProductGroupIdC, PriceProductGroupIdD, PriceProductGroupIdE, PriceProductGroupIdF, Qty, Room, Mounting, Width, WidthB, WidthC, WidthD, WidthE, WidthF, [Drop], DropB, DropC, DropD, DropE, DropF, Roll, RollB, RollC, RollD, RollE, RollF, ControlPosition, ControlPositionB, ControlPositionC, ControlPositionD, ControlPositionE, ControlPositionF, ControlLength, ControlLengthB, ControlLengthC, ControlLengthD, ControlLengthE, ControlLengthF, ControlLengthValue, ControlLengthValueB, ControlLengthValueC, ControlLengthValueD, ControlLengthValueE, ControlLengthValueF, ChainStopper, ChainStopperB, ChainStopperC, ChainStopperD, ChainStopperE, ChainStopperF, FlatOption, FlatOptionB, FlatOptionC, FlatOptionD, FlatOptionE, FlatOptionF, BracketExtension, LinearMetre, LinearMetreB, LinearMetreC, LinearMetreD, LinearMetreE, LinearMetreF, SquareMetre, SquareMetreB, SquareMetreC, SquareMetreD, SquareMetreE, SquareMetreF, TotalItems, Notes, MarkUp, Active) VALUES(@Id, @HeaderId, @ProductId, @FabricId, @FabricIdB, @FabricIdC, @FabricIdD, @FabricIdE, @FabricIdF, @FabricColourId, @FabricColourIdB, @FabricColourIdC, @FabricColourIdD, @FabricColourIdE, @FabricColourIdF, @ChainId, @ChainIdB, @ChainIdC, @ChainIdD, @ChainIdE, @ChainIdF, @BottomId, @BottomIdB, @BottomIdC, @BottomIdD, @BottomIdE, @BottomIdF, @BottomColourId, @BottomColourIdB, @BottomColourIdC, @BottomColourIdD, @BottomColourIdE, @BottomColourIdF, @PriceProductGroupId, @PriceProductGroupIdB, @PriceProductGroupIdC, @PriceProductGroupIdD, @PriceProductGroupIdE, @PriceProductGroupIdF, @Qty, @Room, @Mounting, @Width, @WidthB, @WidthC, @WidthD, @WidthE, @WidthF, @Drop, @DropB, @DropC, @DropD, @DropE, @DropF, @Roll, @RollB, @RollC, @RollD, @RollE, @RollF, @ControlPosition, @ControlPositionB, @ControlPositionC, @ControlPositionD, @ControlPositionE, @ControlPositionF, @ControlLength, @ControlLengthB, @ControlLengthC, @ControlLengthD, @ControlLengthE, @ControlLengthF, @ControlLengthValue, @ControlLengthValueB, @ControlLengthValueC, @ControlLengthValueD, @ControlLengthValueE, @ControlLengthValueF, @ChainStopper, @ChainStopperB, @ChainStopperC, @ChainStopperD, @ChainStopperE, @ChainStopperF, @FlatOption, @FlatOptionB, @FlatOptionC, @FlatOptionD, @FlatOptionE, @FlatOptionF, @BracketExtension, @LinearMetre, @LinearMetreB, @LinearMetreC, @LinearMetreD, @LinearMetreE, @LinearMetreF, @SquareMetre, @SquareMetreB, @SquareMetreC, @SquareMetreD, @SquareMetreE, @SquareMetreF, @TotalItems, @Notes, @MarkUp, 1)", thisConn)
                                                    ...
                                                    thisCmd.Parameters.AddWithValue("@FabricId", If(String.IsNullOrEmpty(fabricId), CType(DBNull.Value, Object), fabricId))
                                                    thisCmd.Parameters.AddWithValue("@FabricIdB", If(String.IsNullOrEmpty(fabricIdB), CType(DBNull.Value, Object), fabricIdB))
                                                    thisCmd.Parameters.AddWithValue("@FabricIdC", If(String.IsNullOrEmpty(fabricIdC), CType(DBNull.Value, Object), fabricIdC))
                                                    thisCmd.Parameters.AddWithValue("@FabricIdD", If(String.IsNullOrEmpty(fabricIdD), CType(DBNull.Value, Object), fabricIdD))
                                                    thisCmd.Parameters.AddWithValue("@FabricIdE", If(String.IsNullOrEmpty(fabricIdE), CType(DBNull.Value, Object), fabricIdE))
                                                    thisCmd.Parameters.AddWithValue("@FabricIdF", If(String.IsNullOrEmpty(fabricIdF), CType(DBNull.Value, Object), fabricIdF))
                                                    thisCmd.Parameters.AddWithValue("@FabricColourId", If(String.IsNullOrEmpty(fabricColourId), CType(DBNull.Value, Object), fabricColourId))
                                                    thisCmd.Parameters.AddWithValue("@FabricColourIdB", If(String.IsNullOrEmpty(fabricColourIdB), CType(DBNull.Value, Object), fabricColourIdB))
                                                    thisCmd.Parameters.AddWithValue("@FabricColourIdC", If(String.IsNullOrEmpty(fabricColourIdC), CType(DBNull.Value, Object), fabricColourIdC))
                                                    thisCmd.Parameters.AddWithValue("@FabricColourIdD", If(String.IsNullOrEmpty(fabricColourIdD), CType(DBNull.Value, Object), fabricColourIdD))
                                                    thisCmd.Parameters.AddWithValue("@FabricColourIdE", If(String.IsNullOrEmpty(fabricColourIdE), CType(DBNull.Value, Object), fabricColourIdE))
                                                    thisCmd.Parameters.AddWithValue("@FabricColourIdF", If(String.IsNullOrEmpty(fabricColourIdF), CType(DBNull.Value, Object), fabricColourIdF))
                                                    thisCmd.Parameters.AddWithValue("@ChainId", If(String.IsNullOrEmpty(chainId), CType(DBNull.Value, Object), chainId))
                                                    thisCmd.Parameters.AddWithValue("@ChainIdB", If(String.IsNullOrEmpty(chainIdB), CType(DBNull.Value, Object), chainIdB))
                                                    thisCmd.Parameters.AddWithValue("@ChainIdC", If(String.IsNullOrEmpty(chainIdC), CType(DBNull.Value, Object), chainIdC))
                                                    thisCmd.Parameters.AddWithValue("@ChainIdD", If(String.IsNullOrEmpty(chainIdD), CType(DBNull.Value, Object), chainIdD))
                                                    thisCmd.Parameters.AddWithValue("@ChainIdE", If(String.IsNullOrEmpty(chainIdE), CType(DBNull.Value, Object), chainIdE))
                                                    thisCmd.Parameters.AddWithValue("@ChainIdF", If(String.IsNullOrEmpty(chainIdF), CType(DBNull.Value, Object), chainIdF))
                                                    thisCmd.Parameters.AddWithValue("@BottomId", If(String.IsNullOrEmpty(bottomId), CType(DBNull.Value, Object), bottomId))
                                                    thisCmd.Parameters.AddWithValue("@BottomIdB", If(String.IsNullOrEmpty(bottomIdB), CType(DBNull.Value, Object), bottomIdB))
                                                    thisCmd.Parameters.AddWithValue("@BottomIdC", If(String.IsNullOrEmpty(bottomIdC), CType(DBNull.Value, Object), bottomIdC))
                                                    thisCmd.Parameters.AddWithValue("@BottomIdD", If(String.IsNullOrEmpty(bottomIdD), CType(DBNull.Value, Object), bottomIdD))
                                                    thisCmd.Parameters.AddWithValue("@BottomIdE", If(String.IsNullOrEmpty(bottomIdE), CType(DBNull.Value, Object), bottomIdE))
                                                    thisCmd.Parameters.AddWithValue("@BottomIdF", If(String.IsNullOrEmpty(bottomIdF), CType(DBNull.Value, Object), bottomIdF))
                                                    thisCmd.Parameters.AddWithValue("@BottomColourId", If(String.IsNullOrEmpty(bottomColourId), CType(DBNull.Value, Object), bottomColourId))
                                                    thisCmd.Parameters.AddWithValue("@BottomColourIdB", If(String.IsNullOrEmpty(bottomColourIdB), CType(DBNull.Value, Object), bottomColourIdB))
                                                    thisCmd.Parameters.AddWithValue("@BottomColourIdC", If(String.IsNullOrEmpty(bottomColourIdC), CType(DBNull.Value, Object), bottomColourIdC))
                                                    thisCmd.Parameters.AddWithValue("@BottomColourIdD", If(String.IsNullOrEmpty(bottomColourIdD), CType(DBNull.Value, Object), bottomColourIdD))
                                                    thisCmd.Parameters.AddWithValue("@BottomColourIdE", If(String.IsNullOrEmpty(bottomColourIdE), CType(DBNull.Value, Object), bottomColourIdE))
                                                    thisCmd.Parameters.AddWithValue("@BottomColourIdF", If(String.IsNullOrEmpty(bottomColourIdF), CType(DBNull.Value, Object), bottomColourIdF))
                                                    thisCmd.Parameters.AddWithValue("@PriceProductGroupId", If(String.IsNullOrEmpty(priceProductGroupId), CType(DBNull.Value, Object), priceProductGroupId))
                                                    thisCmd.Parameters.AddWithValue("@PriceProductGroupIdB", If(String.IsNullOrEmpty(priceProductGroupIdB), CType(DBNull.Value, Object), priceProductGroupIdB))
                                                    thisCmd.Parameters.AddWithValue("@PriceProductGroupIdC", If(String.IsNullOrEmpty(priceProductGroupIdC), CType(DBNull.Value, Object), priceProductGroupIdC))
                                                    thisCmd.Parameters.AddWithValue("@PriceProductGroupIdD", If(String.IsNullOrEmpty(priceProductGroupIdD), CType(DBNull.Value, Object), priceProductGroupIdD))
                                                    ...
                                                    thisCmd.Parameters.AddWithValue("@DropE", dropD)
                                                    thisCmd.Parameters.AddWithValue("@DropF", dropE)
                                                    thisCmd.Parameters.AddWithValue("@Roll", roll)
                                                    thisCmd.Parameters.AddWithValue("@RollB", rollB)
                                                    thisCmd.Parameters.AddWithValue("@RollC", rollC)
                                                    thisCmd.Parameters.AddWithValue("@RollD", rollD)
                                                    thisCmd.Parameters.AddWithValue("@RollE", rollE)
                                                    thisCmd.Parameters.AddWithValue("@RollF", rollF)
                                                    thisCmd.Parameters.AddWithValue("@ControlPosition", controlPosition)
                                                    ...
                                                    thisCmd.Parameters.AddWithValue("@ControlPositionD", controlPositionD)
                                                    thisCmd.Parameters.AddWithValue("@ControlPositionE", controlPositionE)
                                                    thisCmd.Parameters.AddWithValue("@ControlPositionF", controlPositionF)
                                                    thisCmd.Parameters.AddWithValue("@ControlLength", controlLength)
                                                    thisCmd.Parameters.AddWithValue("@ControlLengthB", controlLengthB)
                                                    thisCmd.Parameters.AddWithValue("@ControlLengthC", controlLengthC)
                                                    thisCmd.Parameters.AddWithValue("@ControlLengthD", controlLengthD)
                                                    thisCmd.Parameters.AddWithValue("@ControlLengthE", controlLengthE)
                                                    thisCmd.Parameters.AddWithValue("@ControlLengthF", controlLengthF)
                                                    thisCmd.Parameters.AddWithValue("@ControlLengthValue", controlLengthValue)
                                                    thisCmd.Parameters.AddWithValue("@ControlLengthValueB", controlLengthValueB)
                                                    thisCmd.Parameters.AddWithValue("@ControlLengthValueC", controlLengthValueC)
                                                    thisCmd.Parameters.AddWithValue("@ControlLengthValueD", controlLengthValueD)
                                                    thisCmd.Parameters.AddWithValue("@ControlLengthValueE", controlLengthValueE)
                                                    thisCmd.Parameters.AddWithValue("@ControlLengthValueF", controlLengthValueF)
                                                    thisCmd.Parameters.AddWithValue("@ChainStopper", chainStopper)
                                                    thisCmd.Parameters.AddWithValue("@ChainStopperB", chainStopperB)
                                                    thisCmd.Parameters.AddWithValue("@ChainStopperC", chainStopperC)
                                                    thisCmd.Parameters.AddWithValue("@ChainStopperD", chainStopperD)
                                                    thisCmd.Parameters.AddWithValue("@ChainStopperE", chainStopperE)
                                                    thisCmd.Parameters.AddWithValue("@ChainStopperF", chainStopperF)
                                                    thisCmd.Parameters.AddWithValue("@FlatOption", bottomOption)
                                                    thisCmd.Parameters.AddWithValue("@FlatOptionB", bottomOptionB)
                                                    thisCmd.Parameters.AddWithValue("@FlatOptionC", bottomOptionC)
                                                    thisCmd.Parameters.AddWithValue("@FlatOptionD", bottomOptionD)
                                                    thisCmd.Parameters.AddWithValue("@FlatOptionE", bottomOptionE)
                                                    thisCmd.Parameters.AddWithValue("@FlatOptionF", bottomOptionF)
                                                    thisCmd.Parameters.AddWithValue("@BracketExtension", bracketextension)
                                                    thisCmd.Parameters.AddWithValue("@LinearMetre", linearMetre)
                                                    thisCmd.Parameters.AddWithValue("@LinearMetreB", linearMetreB)
                        ...
                        If Not msgError.InnerText = "" Then
                            Using thisConn As New SqlConnection(myConn)
                                ...
                                Using thisCmd As New SqlCommand("UPDATE OrderHeaders SET Active=0 WHERE Id=@Id",
                                thisConn)
                                    thisCmd.Parameters.AddWithValue("@Id", headerId)
                                    thisCmd.ExecuteNonQuery()
                                End Using

                                Using thisCmd As New SqlCommand("UPDATE OrderDetails SET Active=0 WHERE HeaderId=@Id",
                                thisConn)
                                    thisCmd.Parameters.AddWithValue("@Id", headerId)
                                    thisCmd.ExecuteNonQuery()
                                End Using
    ...
    Protected Sub btnSubmits_Click(sender As Object, e As EventArgs)
        MessageError(False, String.Empty)
        Try
            If ddlCustomer.SelectedValue = "" Then
                MessageError(True, "CUSTOMER NAME IS REQUIRED !")
            ...
            Dim fileExtension As String = IO.Path.GetExtension(fuFile.FileName).ToLower()
            If Not (fileExtension = ".xls" OrElse fileExtension = ".xlsx") Then
                MessageError(True, "THE FILE FORMAT IS NOT SUPPORTED. PLEASE UPLOAD A VALID FILE. !")
                Exit Sub
            End If

            If msgError.InnerText = "" AndAlso (fileExtension = ".xls" Or fileExtension = ".xlsx") Then
                Dim savePath As String = Server.MapPath(String.Format("~/file/cws/{0}", fuFile.FileName.ToString()))
                ...
                Using package As New ExcelPackage(New FileInfo(savePath))
                    ...
                    Dim fileName As String = Path.GetFileName(savePath)
                    Dim newPath As String = Path.Combine(directoryOrder, fileName)
                    ...
                    If Not msgError.InnerText = "" Then
                        Using thisConn As New SqlConnection(myConn)
                            ...
                            Using thisCmd As SqlCommand = New SqlCommand("UPDATE OrderHeaders SET Active=0 WHERE Id=@Id", thisConn)
                                thisCmd.Parameters.AddWithValue("@Id", headerId)
                                thisCmd.ExecuteNonQuery()
                            End Using

                            Using thisCmd As SqlCommand = New SqlCommand("UPDATE OrderDetails SET Active=0 WHERE HeaderId=@Id", thisConn)
                                thisCmd.Parameters.AddWithValue("@Id", headerId)
                                thisCmd.ExecuteNonQuery()
                            End Using
