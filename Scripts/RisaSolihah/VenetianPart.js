...
async function bindItemOrder(itemId, companyDetailId, action) {
    try {
        const response = await $.ajax({
            type: "POST",
            url: "Method.aspx/VenetianPartDetail",
...
function bindValanceType(blindType) {
    return new Promise((resolve, reject) => {
        ...
        const listData = { type: "ValanceType_Part", blindtype: blindType, action: itemAction };
...
function bindValancePosition(blindType) {
    return new Promise((resolve, reject) => {
        ...
        const listData = { type: "ValancePosition_Part", blindtype: blindType, action: itemAction };
...
function bindComponentForm(blindType, colourType) {
    return new Promise((resolve, reject) => {
        ...
        const divsToHide = ["divmounting", "divvalancetype", "divvalancesize", "divreturnposition", "divreturnlength"].map(id => document.getElementById(id));
        ...
        getBlindName(blindType).then(blindName => {
            ...
            if (blindName === "Basswood 50mm Valance" || blindName === "Basswood 63mm Valance" || blindName === "Econo 50mm Valance" || blindName === "Econo 63mm Valance" || blindName === "Ultraslat 50mm Valance" || blindName === "Ultraslat 63mm Valance") {
...
function controlForm(status, isEditItem, isCopyItem) {
    ...
    const inputs = ["blindtype", "colourtype", "qty", "mounting", "valancetype", "valancesizevalue", "returnposition", "returnlengthvalue", "notes", "markup"];
...
function process() {
    ...
    const fields = ["blindtype", "colourtype", "qty", "mounting", "valancetype", "valancesizevalue", "returnposition", "returnlengthvalue", "notes", "markup"];
    ...
    $.ajax({
        type: "POST",
        url: "Method.aspx/VenetianPartProcess",
