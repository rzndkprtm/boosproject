    ...
    return new Promise((resolve, reject) => {
        ...
        if (!blindType || !tubeType) {
            ...
            Promise.all([
                ...
                bindChain(designId, selectedValue)
            ]).then(resolve).catch(reject);
            return;
        }
...
function bindChain(designType, controlType) {
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (Array.isArray(response.d)) {
                    ...
                    if (response.d.length === 1) {
                        chaincolour.selectedIndex = 0;
                    }

                    const selectedValue = chaincolour.value || "";
                    Promise.resolve(
...
function bindFabricColour(fabricType) {
    return new Promise((resolve, reject) => {
        ...
        const listData = { type: "FabricColour", fabrictype: fabricType, orderstatus: orderStatus, rolename: roleAccess, action: itemAction };
...
function bindBottomType(designType) {
    return new Promise((resolve, reject) => {
        ...
        bottomtype.innerHTML = "";

        if (!designType) {
            const selectedValue = bottomtype.value || "";
            Promise.resolve(
                bindBottomColour(selectedValue)
            ).then(resolve).catch(reject);
            return;
        }
            ...
            success: function (response) {
                if (Array.isArray(response.d)) {
                    ...
                    response.d.forEach(function (item) {
                        ...
                        option.value = item.Value;
                        option.text = item.Text;
                        bottomtype.add(option);
                    });

                    if (response.d.length === 1) {
                        bottomtype.selectedIndex = 0;
                    }
...
function controlForm(status, isEditItem, isCopyItem) {
    ...
    const inputs = ["blindtype", "tubetype", "controltype", "colourtype", "qty", "room", "mounting", "fabrictype", "fabriccolour", "roll", "controlposition", "chaincolour", "chainstopper", "controllength", "controllengthvalue", "bottomtype", "bottomcolour", "width", "drop", "markup", "notes", "printing"];
    ...
    inputs.forEach(id => {
        ...
        if (isEditItem) {
            if (id === "qty" || id === "blindtype" || id === "tubetype" || id === "controltype") {
                el.disabled = true;
            } else {
                el.disabled = false;
            }
...
function process() {
    ...
    const fields = ["blindtype", "tubetype", "controltype", "colourtype", "qty", "room", "mounting", "fabrictype", "fabriccolour", "roll", "controlposition", "chaincolour", "chainstopper", "controllength", "controllengthvalue", "bottomtype", "bottomcolour", "width", "drop", "markup", "notes", "printing"];
