...
Partial Class Setting_General_Mailing_Edit
        ...
        Try
            If msgError.InnerText = "" Then
                Using thisConn As New SqlConnection(myConn)
                    Using thisCmd As SqlCommand = New SqlCommand("UPDATE Mailings SET CompanyId=@CompanyId, Name=@Name, Server=@Server, Host=@Host, Port=@Port, NetworkCredentials=@NetworkCredentials, DefaultCredentials=@DefaultCredentials, EnableSSL=@EnableSSL, Account=@Account, Password=@Password, Alias=@Alias, Subject=@Subject, [To]=@To, Cc=@Cc, Bcc=@Bcc, Description=@Description, Active=@Active WHERE Id=@Id", thisConn)
                        thisCmd.Parameters.AddWithValue("@Id", lblId.Text)
                        thisCmd.Parameters.AddWithValue("@CompanyId", If(String.IsNullOrEmpty(ddlCompanyId.SelectedValue), CType(DBNull.Value, Object), ddlCompanyId.SelectedValue))
                        thisCmd.Parameters.AddWithValue("@Name", txtName.Text.Trim())
                        thisCmd.Parameters.AddWithValue("@Server", txtServer.Text.Trim())
                        thisCmd.Parameters.AddWithValue("@Host", txtHost.Text.Trim())
                        thisCmd.Parameters.AddWithValue("@Port", txtPort.Text.Trim())
                        thisCmd.Parameters.AddWithValue("@NetworkCredentials", ddlNetworkCredentials.SelectedValue)
                        thisCmd.Parameters.AddWithValue("@DefaultCredentials", ddlDefaultCredentials.SelectedValue)
                        thisCmd.Parameters.AddWithValue("@EnableSsl", ddlEnableSSL.SelectedValue)
                        thisCmd.Parameters.AddWithValue("@Account", txtAccount.Text.Trim())
                        thisCmd.Parameters.AddWithValue("@Password", txtPassword.Text.Trim())
                        thisCmd.Parameters.AddWithValue("@Alias", txtAlias.Text.Trim())
                        thisCmd.Parameters.AddWithValue("@Subject", txtSubject.Text.Trim())
                        thisCmd.Parameters.AddWithValue("@To", txtTo.Text.Trim())
                        thisCmd.Parameters.AddWithValue("@Cc", txtCC.Text.Trim())
                        thisCmd.Parameters.AddWithValue("@Bcc", txtBcc.Text.Trim())
    ...
    Protected Sub BindCompany()
        ...
        Try
            ddlCompanyId.DataSource = settingClass.GetDataTable("SELECT * FROM Companys WHERE Status='Active' OR Status='Inactive' ORDER BY Name ASC")
