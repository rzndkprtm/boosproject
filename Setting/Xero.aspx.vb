...
Partial Class Setting_Xero
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            txtSearch.Text = Session("SearchXero")
            ...
            If msgErrorProcess.InnerText = "" Then
                ...
                If lblAction.Text = "Add" Then
                    ...
                    Using thisConn As New SqlConnection(myConn)
                        Using thisCmd As SqlCommand = New SqlCommand("INSERT INTO Xeros VALUES (@Id, @Name, @ItemCode, @AccountCode, @Description, @Active)", thisConn)
                            thisCmd.Parameters.AddWithValue("@Id", thisId)
                            thisCmd.Parameters.AddWithValue("@Name", txtName.Text.Trim())
                            thisCmd.Parameters.AddWithValue("@ItemCode", txtItemCode.Text)
                            thisCmd.Parameters.AddWithValue("@AccountCode", txtAccountCode.Text)
                ...
                If lblAction.Text = "Edit" Then
                    Using thisConn As New SqlConnection(myConn)
                        Using thisCmd As SqlCommand = New SqlCommand("UPDATE Xeros SET Name=@Name, ItemCode=@ItemCode, AccountCode=@AccountCode, Description=@Description, Active=@Active WHERE Id=@Id", thisConn)
    ...
    Protected Sub btnDelete_Click(sender As Object, e As EventArgs)
        ...
        Try
            ...
            Using thisConn As New SqlConnection(myConn)
                Using thisCmd As SqlCommand = New SqlCommand("DELETE FROM Xeros WHERE Id=@Id; DELETE FROM Logs WHERE Type='Xeros' AND DataId=@Id;", thisConn)
                    thisCmd.Parameters.AddWithValue("@Id", thisId)
                    thisConn.Open()
                    thisCmd.ExecuteNonQuery()
                End Using
    ...
    Protected Sub BindData(searchText As String)
        Session("SearchXero") = String.Empty
        Try
            Dim search As String = String.Empty
            If Not searchText = "" Then
                search = "WHERE Id LIKE '%" & searchText.Trim() & "%' OR Name LIKE '%" & searchText.Trim() & "%' OR ItemCode LIKE '%" & searchText.Trim() & "%' OR AccountCode LIKE '%" & searchText.Trim() & "%'"
            End If
            Dim thisString As String = String.Format("SELECT *, CASE WHEN Active=1 THEN 'Yes' WHEN Active=0 THEN 'No' ELSE 'Error' END AS DataActive FROM Xeros {0} ORDER BY Name ASC", search)
